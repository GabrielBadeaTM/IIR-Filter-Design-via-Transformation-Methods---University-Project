%
%	File BUT_FTI_PSEUDOTUSTIN.M
%
%	Function: BUT_FTI_PSEUDOTUSTIN
%
%	Synopsis: [B,A,M_found,W_c] = But_FTI_PseudoTustin(w_p,w_s,Delta_p,Delta_s,Ts) ; 
%
%	Designs a low-pass discrete IIR filter of Butterworth class, 
%	by solving a design problem with specified tolerances. 
%	The design is realized by means of a pseudo-Tustin discretizing method.
%
%   Similar to the original BUT_FTI function, but uses a pseudo-Tustin approach
%   and also outputs minimal order M found and W_c calculated.
%
%
...
%   Modified: Decembrie 25, 2025
%

function [B,A,M_found,W_c] = But_FTI_PseudoTustin(w_p,w_s,Delta_p,Delta_s,Ts)
...
%
	...
	w_p = 1*tan(w_p*pi/2)/Ts ; 	% Compute Omega_p.
	w_s = 1*tan(w_s*pi/2)/Ts ; 	% Compute Omega_s.
